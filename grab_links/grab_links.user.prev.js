...
// @author             ollily2907
// @version            1.00.06
// @date               $LastChangedDate: 2013-03-11 12:53:39 +0100 (Mo, 11 Mrz 2013) $
...
/*
 IMPORTANT NOTICE:
 To use the clipboard support you'll have enable it, but be informed, that this
...
/**
 * Shorthand, tries to find an element in the page.
 *
 * @param Identifier -
...
/**
 * Tries to find an element in the page.
 *
 * @param Mode -
...
/**
 * Verifies if object is an array.
 *
 * @param obj -
 *            the object to test
 ...
 * @return {Array} an array with all found links
 */
function gm_findLinksInPage(sea, withDesc) {
	...
	for ( var i = 0; i < document.links.length; i++) {
		...
		var found = searchText.search(sea) != -1;
		// alert(searchText + " " + sea + " " + found);

		if (withDesc != 0) {
			if (!found && curlink.alt != null) {
				searchText = curlink.alt;
				found = searchText.search(sea) != -1;
			}
			if (!found && curlink.title != null) {
				searchText = curlink.title;
				found = searchText.search(sea) != -1;
			}
			if (!found && curlink.mouseover != null) {
				searchText = "" + curlink.mouseover;
				try {
					found = searchText.search(sea) != -1;
				} catch (e) {
					// ignored
			...
			if (!found && curlink.onclick != null) {
				searchText = "" + curlink.onclick;
				try {
					found = searchText.search(sea) != -1;
				} catch (e) {
					// ignored
		...
		if (found) {
			if (gm_getAttributeI(curlink.id, FL_TAG) != FL_ID) {
				...
				if (withDesc != 0) {
					...
					if (curlink.title != null) {
						htmltext += "\n" + curlink.title;
					}
					if (curlink.mouseover != null) {
						try {
							htmltext += "\n" + curlink.mouseover;
						} catch (e) {
							// ignored
 ...
 *
 * @see {@link #gm_getTextSelectMode()}
 */
var SELECT_CURR = gm_getTextSelectMode();
...
/**
 * Selects or Unselects the content inside an element.
 *
 * @param elem -
...
/**
 * Sorts an array by a specific sort order.
 *
 * @param unsortedArray -
...
/**
 * Defines the primary action, which should be done after creating the controls.
...
function lgm_showLinks(sea) {
	...
	if (bTestMode) {
		...
		pagelinks = gm_findLinksInPage(sea, 1);
	}
	...
	for ( var i = 0; i < pagelinks.length; i++) {
		...
		// row for htmllink
		var alink = "<a id=\'" + scriptID + i + "\' " + FL_TAG + "=\'" + FL_ID + "\' title=\'" + curcap + "\' href=\'" + curlink
				+ "\' target=\'_blank\'>" + curcap + "<\/a><br\/>\n";
		var pcap = curcap + "<br\/>";
